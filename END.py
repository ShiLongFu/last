...
# Fibrinogen: 数值输入
Fibrinogen = st.number_input("Fibrinogen (g/L):", min_value=0, max_value=20, value=1)
...
if st.button("Predict"):
    ...
    if predicted_class == 1:
        advice = (f"According to our predictive model, this patient with PADBS has a high risk of END. " f"The XGBoost model predicts that the probability of experiencing END is {probability:.1f}%.")
    else:
        advice = (f"According to our predictive model, this patient with PADBS has a low risk of END. " f"The XGBoost model predicts that the probability of not experiencing END is {probability:.1f}%.")
